...
#             text="Hello! I am Tushar from Codeyoung. How are you doing today?", allow_interruptions=True
#         )
...
class VoiceServiceAgent(Agent, BaseCustomerServiceAgent):
    ...
    async def send_message(self, message: str, message_type: str = "text"):
        """Implementation for voice agent - uses TTS"""
        if hasattr(self, 'session') and self.session:
            # For voice, we speak the message
            if message_type == "tool_start":
                # Don't speak tool start messages in voice mode
                pass
            elif message_type in ["tool_success", "tool_error"]:
                # Don't speak detailed tool results in voice mode
                pass
            else:
                # Speak regular text messages
                await self.session.say(text=message)
        else:
            logger.warning("Cannot send voice message: session not available")
...
class ChatServiceAgent(BaseCustomerServiceAgent):
    ...
    async def send_message(self, message: str, message_type: str = "text"):
        ...
        if not self.participant or not self.session:
            logger.warning("Cannot send message: participant or session not set")
            return
    ...
    async def end_chat_session(self, ctx: RunContext):
        """End the chat session gracefully"""
        participant_id = self.participant.identity if self.participant else 'unknown'
    ...
    async def end_session(self, ctx: RunContext):
        """Implementation of abstract method"""
        return await self.end_chat_session(ctx)
    ...
    async def on_enter(self):
        ...
        # Set agent attribute in room
        from .data_entities import UserData
        if hasattr(self, 'session') and hasattr(self.session, 'userdata'):
            userdata: UserData = self.session.userdata
            if userdata.ctx and userdata.ctx.room:
                await userdata.ctx.room.local_participant.set_attributes(
