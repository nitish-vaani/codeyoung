...
"""
Session and agent setup helpers with chat support.
Handles session configuration, background audio, and recording setup for both voice and chat.
"""
...
class ChatSession:
    ...
    def on(self, event_name: str, handler):
        ...
        if event_name == "conversation_item_added":
            # For chat sessions, we handle conversation items differently
            # Store the handler to call it manually when needed
    ...
    async def generate_reply(self, instructions: str):
        ...
        # This would integrate with LLM for chat responses
        # For now, just send the instruction as response
        if self._agent:
