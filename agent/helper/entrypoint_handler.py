...
# """
# Updated entrypoint handler using combined agent classes.
# Routes to appropriate agent based on metadata with client-agnostic naming.
# """
...
#         def on_data_received(data_packet: rtc.DataPacket, participant: rtc.RemoteParticipant):
...
#             # Create chat session instead of agent session
#             session = create_chat_session(userdata, config, agent_config)
#             agent.set_session(session)
            
#             # Start session for chat
...
#     # Setup transcript persistence AFTER session is created
#     if config["store_transcription"]['switch']:
#         finish_queue = transcript_manager.setup_transcript_persistence(
