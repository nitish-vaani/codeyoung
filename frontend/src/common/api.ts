...
export const startFloatingChat = (chatReq: {
  user_id: string;
  customer_name: string;
  agent_id?: string;
}) => {
  return axios.post('/floating-chat/start', chatReq, {
    timeout: 20000
  })
}

export const endFloatingChat = (sessionId: string) => {
  return axios.post(`/floating-chat/end/${sessionId}`, {}, {
    timeout: 20000
  })
}

export const saveFloatingChatMessage = (messageReq: {
  session_id: string;
  type: string;
  content: string;
  sender: string;
  ...
  return axios.post('/floating-chat/message', messageReq, {
    timeout: 20000
  })
}

export const getFloatingChatHistory = (user_id: string) => {
  return axios.get(`/floating-chat/history/${user_id}`, {
    timeout: 20000
  });
}

export const getFloatingChatMessages = (session_id: string) => {
  return axios.get(`/floating-chat/messages/${session_id}`, {
    timeout: 20000
  });
}
